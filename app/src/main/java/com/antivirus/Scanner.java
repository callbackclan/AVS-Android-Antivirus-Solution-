...
public class Scanner {
    ...
    public List<String> getInfectedApps() {
        ...
        new VirusAppDB(this.context);

        List<Application> virus = util.getVirus(this.context);
        for (int i = 0; i < virus.size(); i++) {
            for (int j = 0; j < this.applicationsList.size(); j++) {
                if (virus.get(i).name.equalsIgnoreCase(this.applicationsList.get(j).name)) {
                    infectedAppResult.add(this.applicationsList.get(j).name);
                }
            }
        }
